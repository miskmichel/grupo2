...
| 3 | Train ≥2 models | ✅ 5 approaches — see [Methodology](METHODOLOGY.md) |
| 4 | Evaluate & compare | ✅ [Comparison report](docs/step-04-model-comparison.md) |
| 5 | Synthesis | ✅ Final cell in [`codigo.ipynb`](codigo.ipynb) |
...
- **Five modeling approaches land within ~0.008 test ROC-AUC of each other** — GradientBoosting (0.8440), Logistic Regression (0.8424), LinearSVC (0.8399), Naive Bayes (0.8389), KNN (0.8363), plus teammate Caco's independently-built BernoulliNB — all inside the Step 1 predicted 0.84–0.85 band, all under one CV standard deviation apart. Model family barely matters here once each is properly tuned; see [Methodology](METHODOLOGY.md#step-3--modeling).
...
- **Shipped model: Logistic Regression**, ROC-AUC 0.8424, F1(Yes) 0.6178 — chosen for interpretability given a statistical tie with GradientBoosting, and because teammate Caco's fully independent pipeline reached the same conclusion. See [Step 4 comparison](docs/step-04-model-comparison.md) and the synthesis cell in [`codigo.ipynb`](codigo.ipynb).
